...
class MideaDevice {
    constructor(options) {
        this.log = options.log || { debug: console.log, info: console.log, error: console.error };
        this.log.debug('[MideaDevice] Konstruktor aufgerufen mit: ip=' + options.ip + ', token=' + options.token + ', key=' + options.key + ', protocol=' + (options.protocol || 3));
        this.ip = options.ip;
        ...
        this.log.debug('[MideaDevice] Konstruktor fertig');
    }

    async connect(refreshStatus = true) {
        this.log.debug('[MideaDevice] connect() aufgerufen');
        return new Promise((resolve, reject) => {
            this._socket = new net.Socket();
            this._socket.setTimeout(10000);
            this._socket.connect(this.port, this.ip, async () => {
                this.log.info('[MideaDevice] Socket connected');
                let triedProtocol2 = false;
                let lastError = null;
                    ...
                    try {
                        ...
                        if (refreshStatus) {
                            try {
                                ...
                                this.log.error('[MideaDevice] Fehler bei refreshStatus: ' + e);
                            }
                        }
                        this._available = true;
                        this.log.info('[MideaDevice] connect() erfolgreich, resolve()');
                        return resolve(true);
                    } catch (e) {
                        lastError = e;
                        this.log.warn('[MideaDevice] authenticate() mit Protokoll ' + this.protocol + ' fehlgeschlagen: ' + e);
                        if (this.protocol === 3 && !triedProtocol2) {
                            this.log.info('[MideaDevice] Versuche Fallback auf Protokoll 2...');
                            this.protocol = 2;
                            triedProtocol2 = true;
                            continue;
                        } else {
                            this._available = false;
                            this.log.error('[MideaDevice] Fehler bei authenticate: ' + e);
                            return reject(e);
                        }
                    }
                }
                ...
                this._available = false;
                this.log.error('[MideaDevice] Fehler bei authenticate (beide Protokolle): ' + lastError);
                return reject(lastError);
            });
            this._socket.on('error', (err) => {
                this._available = false;
                this.log.error('[MideaDevice] Socket error: ' + err);
                reject(err);
            });
        });
    }
    ...
    async authenticate() {
        this.log.debug('[MideaDevice] authenticate() aufgerufen');
        // Pad token to 128 bytes for handshake (as in HomeAssistant)
        let tokenPadded = Buffer.alloc(128, 0);
        this.token.copy(tokenPadded, 0, 0, Math.min(this.token.length, 128));
        const request = this._security.encode_8370(tokenPadded, MSGTYPE_HANDSHAKE_REQUEST);
        this.log.debug('[MideaDevice] Sending handshake request (hex): ' + request.toString('hex'));
        this._socket.write(request);
        return new Promise((resolve, reject) => {
            let timeout = setTimeout(() => {
                this.log.error('[MideaDevice] authenticate() Timeout: Keine Antwort vom Gerät');
                reject(new Error('AuthTimeout'));
            }, 8000);
            this._socket.once('data', (response) => {
                clearTimeout(timeout);
                this.log.debug('[MideaDevice] authenticate() Daten empfangen, Länge: ' + response.length);
                this.log.debug('[MideaDevice] Received handshake response (hex): ' + response.toString('hex'));
                if (response.length < 20) {
                    this.log.error('[MideaDevice] authenticate() Antwort zu kurz!');
                    return reject(new Error('AuthException'));
                }
                const resp = response.slice(8, 72);
                try {
                    this._security.tcp_key(resp, this.key);
                } catch (e) {
                    this.log.error('[MideaDevice] Fehler bei tcp_key: ' + e);
                    return reject(e);
                }
                this.log.debug('[MideaDevice] authenticate() erfolgreich');
                resolve();
            });
            // Also log any data received, even if not expected
            this._socket.on('data', (data) => {
                this.log.debug('[MideaDevice] (on data) Received: ' + data.toString('hex'));
            });
        });
    }

    async refreshStatus(waitResponse = false) {
        // Dummywerte für Testzwecke setzen
        this.attributes = {
            ...
            total_energy_consumption: 13.7
        };
        // Logge die Dummywerte
        console.log('Dummy-Status gesetzt:', this.attributes);
        ...
        for (const message of messages) {
            ...
            if (message.length > 56) {
                ...
                if (cryptographic.length % 16 === 0) {
                    ...
                    // 3. Statusbytes parsen (nach HomeAssistant-Logik)
                    if (decrypted.length > 20) {
                        // Power
                        this.attributes.power = (decrypted[1] & 0x1) > 0;
                        // Modus
                        const modeVal = (decrypted[2] & 0xe0) >> 5;
                        const modeMap = ['auto', 'cool', 'dry', 'heat', 'fan_only', 'eco'];
                        this.attributes.mode = modeMap[modeVal] || 'unknown';
                        // Zieltemperatur
                        this.attributes.target_temperature = (decrypted[2] & 0x0F) + 16.0 + ((decrypted[2] & 0x10) ? 0.5 : 0.0);
                        // Lüfterstufe
                        this.attributes.fan_speed = decrypted[3] & 0x7F;
                        // Swing
                        this.attributes.swing_vertical = (decrypted[7] & 0x0C) > 0;
                        this.attributes.swing_horizontal = (decrypted[7] & 0x03) > 0;
                        // Boost/Turbo
                        this.attributes.boost_mode = ((decrypted[8] & 0x20) > 0) || ((decrypted[10] & 0x2) > 0);
                        // ECO
                        this.attributes.eco_mode = (decrypted[9] & 0x10) > 0;
                        // Sleep
                        this.attributes.sleep_mode = (decrypted[10] & 0x01) > 0;
                        // Indoor-Temperatur
                        if (decrypted[11] !== 0xFF) {
                            const temp_integer = ((decrypted[11] - 50) / 2);
                            const temp_decimal = (decrypted[15] & 0x0F) * 0.1;
                            this.attributes.indoor_temperature = (decrypted[11] > 49) ? (temp_integer + temp_decimal) : (temp_integer - temp_decimal);
                        } else {
                            this.attributes.indoor_temperature = null;
                        }
                        // Outdoor-Temperatur
                        if (decrypted[12] !== 0xFF) {
                            const temp_integer = ((decrypted[12] - 50) / 2);
                            const temp_decimal = ((decrypted[15] & 0xF0) >> 4) * 0.1;
                            this.attributes.outdoor_temperature = (decrypted[12] > 49) ? (temp_integer + temp_decimal) : (temp_integer - temp_decimal);
                        } else {
                            this.attributes.outdoor_temperature = null;
                        }
                        // Luftfeuchtigkeit
                        this.attributes.indoor_humidity = decrypted[17];
                        // Energieverbrauch (XC1MessageBody, falls vorhanden)
                        if (decrypted.length > 22 && decrypted[3] === 0x44) {
                            // total_energy_consumption: 4 Bytes ab decrypted[4]
                            this.attributes.total_energy_consumption = ((decrypted[4] << 24) | (decrypted[5] << 16) | (decrypted[6] << 8) | decrypted[7]) / 100.0;
                        }
